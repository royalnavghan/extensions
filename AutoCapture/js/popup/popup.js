    ...
    if (chrome.sidePanel) {
      chrome.windows.getCurrent(function(window) {
        chrome.sidePanel.open({
          ...
          handleError('Side Panel Error', error.message);
        });
        
        chrome.storage.local.set({displayMode: 'sidePanel'});
      });
    } else {
      handleError('Side Panel Not Available', 
        'Chrome side panel API is not available. Try updating Chrome to the latest version.');
    }
  }
